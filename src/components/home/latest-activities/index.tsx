...
import { Skeleton as AntdSkeleton, Card, List, Space } from 'antd';
...
interface DashboardLatestActivitiesProps {
  limit?: number;
}

export const DashboardLatestActivities = ({ limit = 5 }: Readonly<DashboardLatestActivitiesProps>) => {
  const {
    data: audit,
          ...
          renderItem={(_item, index) => {
            return (
              <List.Item key={index}>
                <List.Item.Meta
                  avatar={
                    <AntdSkeleton.Avatar active size={48} shape="square" style={{ borderRadius: '4px' }} />
                  }
                  title={<AntdSkeleton.Button active style={{ height: '16px' }} />}
                  description={<AntdSkeleton.Button active style={{ width: '300px', height: '16px' }} />}
                />
              </List.Item>
            );
          }}
